...
async function handleApi(req, res, pathname, method) {
  ...
  try {
    ...
    if (resource === 'trips') {
      if (method === 'POST') {
        const body = await parseBody(req);
        const trip = db.createTrip(body);
        return sendJSON(res, { success: true, data: trip }, 201);
      }
      const query = url.parse(req.url, true).query;
      return sendJSON(res, db.getTrips(query.status));
    }
    ...
    if (resource === 'reminders') {
      if (method === 'POST') {
        const body = await parseBody(req);
        const reminder = db.createReminder(body);
        return sendJSON(res, { success: true, data: reminder }, 201);
      }
      if (method === 'PUT' && id) {
        const body = await parseBody(req);
        if (body.toggle) {
          const r = db.toggleReminder(id);
          if (!r) return sendError(res, '提醒不存在', 404);
          return sendJSON(res, { success: true, data: r });
        }
        const r = db.updateReminder(id, body);
        if (!r) return sendError(res, '提醒不存在', 404);
        return sendJSON(res, { success: true, data: r });
      }
      if (method === 'DELETE' && id) {
      ...
      return sendJSON(res, db.getReminders());
    }
