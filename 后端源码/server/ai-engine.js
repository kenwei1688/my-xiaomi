...
class AIEngine {
  ...
  extractEntities(msg, intent) {
    ...
    if (intent.type === 'add_reminder') {
      // 提取时间 HH:MM（支持 8:30 / 8点30 / 下午3点 / 晚上8点 等）
      let rHour = null, rMinute = 0;
      const colonMatch = msg.match(/(\d{1,2})[:：](\d{0,2})/);
      const dianMatch = msg.match(/(\d{1,2})\s*点\s*(\d{0,2})/);
      const periodMatch = msg.match(/(早上|上午|中午|下午|傍晚|晚上)\s*(\d{1,2})/);
      if (colonMatch) {
        rHour = parseInt(colonMatch[1], 10);
        rMinute = parseInt(colonMatch[2] || '0', 10) || 0;
      } else if (dianMatch) {
        rHour = parseInt(dianMatch[1], 10);
        if (dianMatch[2]) rMinute = parseInt(dianMatch[2], 10) || 0;
      } else if (periodMatch) {
        rHour = parseInt(periodMatch[2], 10);
      }
      if (rHour !== null) {
        // 下午/晚上/傍晚 12 小时制转换
        if (/(下午|傍晚|晚上)/.test(msg)) {
          rHour += 12;
          if (rHour > 23) rHour -= 24;
        }
        if (rHour >= 0 && rHour <= 23 && rMinute >= 0 && rMinute <= 59) {
          entities.reminderTime = String(rHour).padStart(2, '0') + ':' + String(rMinute).padStart(2, '0');
        }
      }
      ...
      if (/(上班|打卡上班|早起|起床)/.test(msg)) {
        ...
        // 尝试从消息中提取提醒内容
        const remindContent = msg
          .replace(/提醒我|提醒一下|设个提醒|设置提醒|帮我提醒|加个提醒|记得提醒|别忘了/g, '')
          .replace(/用?(闹钟|短信|微信)(提醒|通知)/g, '')
          .replace(/明天|今天|后天|早上|上午|下午|晚上|\d+[:：点]\d*|点/g, '')
          .trim();
        entities.reminderTitle = remindContent.substring(0, 20) || '自定义提醒';
      }

      // 提取提醒描述
      entities.reminderDesc = msg
        .replace(/提醒我|提醒一下|设个提醒|设置提醒|帮我提醒|加个提醒/g, '')
        .replace(/用?(闹钟|短信|微信)(提醒|通知)/g, '')
        .trim()
        .substring(0, 60);
      ...
      // 提取提醒方式（闹钟 / 短信 / 微信）
      if (/(短信|发短信|短信提醒|手机短信|短信通知)/.test(msg)) {
        entities.reminderMethod = 'sms';
      } else if (/(微信|发微信|微信提醒|微信通知)/.test(msg)) {
        entities.reminderMethod = 'wechat';
      } else if (/(闹钟|响铃|闹铃|闹钟提醒)/.test(msg)) {
        entities.reminderMethod = 'alarm';
      }
    }
  ...
  handlePlanTrip(entities, session, originalMsg) {
    ...
    if (session.userId && this.store) {
      newTrip = this.store.createTrip(session.userId, tripData);
    } else {
      newTrip = this.db.createTrip(tripData);
    }
  ...
  handleAddReminder(entities, session, originalMsg) {
    ...
    const reminderMethod = entities.reminderMethod || 'alarm';

    // ===== 没有提取到时间 → 返回草稿，请用户补充时间 + 提醒方式 =====
    if (!entities.reminderTime) {
      return {
        reply: `好的，我来帮您设置${reminderTitle}～不过还差一个关键信息：提醒时间。`,
        actions: ['设置提醒时间'],
        askReminder: {
          title: reminderTitle,
          type: reminderType,
          ...
          time: '',
          tip: '请设置提醒时间，并选择提醒方式（闹钟提醒 / 短信提醒 / 微信提醒）：'
        }
      };
    }

    const reminderTime = entities.reminderTime;

    // 调用数据库创建提醒（登录用户落云端，未登录回退内存演示库）
    const reminderData = {
      type: reminderType,
      title: reminderTitle,
      desc: reminderDesc,
    ...
    let newReminder;
    if (session.userId && this.store) {
      newReminder = this.store.createReminder(session.userId, reminderData);
    } else {
      newReminder = this.db.createReminder(reminderData);
    }
