...
// ============================================================
// 业务时区：固定 Asia/Shanghai (UTC+8)
// 云端(Railway)跑在 UTC，若直接用本地时区方法构造时间，用户感知会偏移 8 小时。
// 统一用下面这套"北京时间日历字段 ↔ UTC 时刻"的转换，保证本地/云端行为一致。
// ============================================================
...
// 用"北京时间日历字段"构造真实 UTC 时刻
export function cnDate(y, m, day, h = 0, mi = 0, s = 0) {
  return new Date(Date.UTC(y, m - 1, day, h - TZ_OFFSET_H, mi, s, 0));
}

// 北京时间下的当天零点
export function startOfDay(d) {
  const p = cnParts(d);
  return cnDate(p.y, p.m, p.day, 0, 0, 0);
}

// 北京时间下加减天数（保持时分秒）
export function addDays(d, n) {
  const p = cnParts(d);
  return cnDate(p.y, p.m, p.day + n, p.h, p.mi, p.s);
}

// 在保持北京时间"日期"不变的前提下设置时分
export function cnSetTime(d, h, mi, s = 0) {
  const p = cnParts(d);
  return cnDate(p.y, p.m, p.day, h, mi, s);
}

// 格式化为北京时间可读串
export function cnFormat(d) {
  const p = cnParts(d);
  return `${p.y}-${pad(p.m)}-${pad(p.day)} ${pad(p.h)}:${pad(p.mi)}`;
}
...
export function parseDateTime(text, now = new Date()) {
  ...
  // 明确日期 YYYY-MM-DD / YYYY/MM/DD
    ...
    base = cnDate(parseInt(m[1], 10), parseInt(m[2], 10), parseInt(m[3], 10));
    hasExplicitDate = true;
  }
  // M月D日 / M月D号 / M/D / M.D
  else if (/(\d{1,2})[月.\/](\d{1,2})[日号]?/.test(text)) {
    const m = text.match(/(\d{1,2})[月.\/](\d{1,2})[日号]?/);
    const ny = cnParts(now).y;
    base = cnDate(ny, parseInt(m[1], 10), parseInt(m[2], 10));
    if (base < startOfDay(now)) base = cnDate(ny + 1, parseInt(m[1], 10), parseInt(m[2], 10));
    hasExplicitDate = true;
  }
  ...
  if (hour !== null) {
    let d = cnSetTime(base, hour, minute);
    ...
    const d = cnSetTime(base, 9, 0);
    result.iso = d.toISOString();
    result.dateOnly = true;
  } else {
    // 完全没时间信息，但若有重复模式则算下一次触发
    if (result.repeat) {
      const np = cnParts(now);
      let nb = cnSetTime(startOfDay(now), 9, 0);
      if (result.repeat === 'monthly' && result.repeatMonthDay) {
        const md = Math.min(result.repeatMonthDay, 28);
        let m = np.m, y = np.y;
        let cand = cnDate(y, m, md, 9, 0);
        if (cand <= now) { m++; if (m > 12) { m = 1; y++; } cand = cnDate(y, m, md, 9, 0); }
        nb = cand;
      } else if (result.repeat === 'weekly' && result.repeatWeekday != null) {
        let diff = (result.repeatWeekday - np.dow + 7) % 7;
        if (diff === 0 && nb <= now) diff = 7;
        nb = addDays(nb, diff);
      } else if (result.repeat === 'weekday') {
        while (nb <= now || cnParts(nb).dow === 0 || cnParts(nb).dow === 6) { nb = cnSetTime(addDays(nb, 1), 9, 0); }
      } else {
        // daily
...
function formatLabel(d, repeat, wk, md) {
  if (!d) return '';
  const p = cnParts(d);
  const datePart = `${p.m}月${p.day}日(${WEEKDAY_CN[p.dow]})`;
  const timePart = `${pad(p.h)}:${pad(p.mi)}`;
