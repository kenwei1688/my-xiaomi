...
import {
  detectIntent, parseDateTime, extractDestination, extractDays, extractCityPair, cleanText, extractMethod,
  cnParts, cnDate
} from './nlu.js';
...
const REMINDER_CATEGORIES = {
  clockin: { label: '上班打卡', icon: '🕘' },
  clockout: { label: '下班打卡', icon: '🌆' },
  birthday: { label: '生日', icon: '🎂' },
  meeting: { label: '会议', icon: '📋' },
  travel: { label: '出行', icon: '🧳' },
  business_trip: { label: '出差', icon: '✈️' },
  repayment: { label: '还钱', icon: '💰' },
  general: { label: '普通提醒', icon: '🔔' }
};
const REMINDER_METHODS = {
  alarm: { label: '闹钟提醒', icon: '⏰' },
  wechat: { label: '微信提醒', icon: '💬' },
  sms: { label: '短信提醒', icon: '📱' }
};
...
function extractEventTitle(text) {
  return text
    ...
    .replace(/(提醒我|提醒|记得到|记得|别忘了|别忘记|闹钟|定个时|到点|备忘|通知我|帮我|请|麻烦|我想|我要|可以|能否|能不能|用微信|用短信|微信提醒|短信提醒|闹钟提醒|微信|短信)/g, '')
    .replace(/(是|的|了)/g, '')
    .replace(/[，,。.！!？?、~～]/g, ' ')
    .replace(/\s+/g, ' ')
    .trim();
...
function buildDiaryContent(topic, mood, dateISO, isTrip) {
  const dp = cnParts(dateISO ? new Date(dateISO) : new Date());
  const label = `${dp.m}月${dp.day}日`;
...
function runQuickCommand(cmd, text, userId) {
  ...
  if (a === 'reminder') {
    ...
    const r = createReminder(cmd.payload?.title || cmd.title, dt.iso, {
      category: cmd.payload?.category || 'general', method: cmd.payload?.method || 'alarm'
    }, userId);
...
function bookingFlow(type, text, prefill = {}) {
  ...
  if (dm && !pref.date && !pref.checkin) {
    const y = dm[1].length === 4 ? dm[1] : String(cnParts(now()).y);
...
function createReminder(title, iso, extra, userId) {
  ...
  const r = {
    ...
    category: extra.category || 'general', method: extra.method || 'alarm',
...
function reminderFlow(text, tag, userId) {
  ...
  const method = extractMethod(text) || 'alarm';
  const methodLabel = REMINDER_METHODS[method].label;
  const cat = REMINDER_CATEGORIES[tag] ? tag : 'general';

  // ---- 上班/下班打卡提醒（clockin / clockout / commute）----
  // 规则：时间点必须由用户明确给出（如 8:30 / 18:30 / 9点），未给则反问、不创建
  if (tag === 'commute' || tag === 'clockin' || tag === 'clockout') {
    const isOut = tag === 'clockout' || (tag === 'commute' && /下班/.test(text) && !/上班/.test(text));
    const category = isOut ? 'clockout' : 'clockin';
    const title = isOut ? '下班打卡提醒' : '上班打卡提醒';
    const hasTime = /(\d{1,2})\s*点|(\d{1,2}):(\d{2})|(\d+)\s*个?小时[以]?后|(\d+)\s*分钟[以]?后/.test(text);
    if (!hasTime) {
      return emptyReply(
        `${REMINDER_CATEGORIES[category].icon} 几点${isOut ? '下班打卡' : '上班打卡'}呢？告诉我具体时间，我才能帮你设置。例如「每天 ${isOut ? '18:30' : '09:00'} ${isOut ? '下班打卡' : '上班打卡'}」或「明天早上8:30上班打卡」。`,
        { suggestions: [`每天 ${isOut ? '18:30' : '09:00'} ${isOut ? '下班打卡' : '上班打卡'}`, `明天早上${isOut ? '18:30' : '09:00'}${isOut ? '下班打卡' : '上班打卡'}`, '查看我的提醒'] }
      );
    }
    const base = new Date(parsed.iso || now());
    const bp = cnParts(base);
    const timeLabel = `${pad2(bp.h)}:${pad2(bp.mi)}`;
    const repeat = parsed.repeat || 'daily';
    const r = createReminder(title, base.toISOString(), { repeat, category, method, source: 'chat' }, userId);
    db.forUser(userId).pushNotification({ text: `已添加${title}（${timeLabel}，${methodLabel}）`, level: 'success' });
    return emptyReply(
      `已为你设置「${title}」${REMINDER_CATEGORIES[category].icon} ${timeLabel} · ${methodLabel}，已加入「我的」页面。`,
      { results: [r], suggestions: ['查看我的提醒', '换成微信提醒'] }
    );
  }
  ...
  if (tag === 'birthday') {
    ...
    if (m) {
      ...
      const thisYear = cnParts(now()).y;
      let d = cnDate(thisYear, month, day, 9, 0);
      if (d < now()) d = cnDate(thisYear + 1, month, day, 9, 0);
      const r = createReminder(`${title}生日`, d.toISOString(), { repeat: 'yearly', repeatMonthDay: day, category: 'birthday', method, source: 'chat' }, userId);
      db.forUser(userId).pushNotification({ text: `已添加每年${month}月${day}日「${title}生日」提醒（${methodLabel}）`, level: 'success' });
      return emptyReply(`已添加「每年 ${month}月${day}日 ${title}生日」提醒 🎂 ${methodLabel}，已加入「我的」页面。`, { results: [r] });
    }
  ...
  if (tag === 'repayment') {
    ...
    title = (title && title !== '提醒') ? (/还/.test(title) ? title : `还钱·${title}`) : '还钱提醒';
    if (!parsed.iso) {
      return emptyReply('什么时候需要还呢？告诉我日期，例如「每月15号提醒我还信用卡」「下周五提醒我还钱」💰', {
        suggestions: ['每月15号提醒我还信用卡', '每月10号提醒我还花呗']
      });
    }
    const r = createReminder(title, parsed.iso, {
      repeat: parsed.repeat, repeatWeekday: parsed.repeatWeekday, repeatMonthDay: parsed.repeatMonthDay,
      category: 'repayment', method, source: 'chat'
    }, userId);
    db.forUser(userId).pushNotification({ text: `已添加还钱提醒：${title}（${parsed.label}）`, level: 'success' });
    return emptyReply(`已添加还钱提醒：「${title}」💰 ${parsed.label} · ${methodLabel}，已加入「我的」页面。`, {
      results: [r], suggestions: ['查看我的提醒', '再添加一个']
    });
  }
  ...
  // ---- 通用提醒（meeting / travel / business_trip / general）----
  let title = extractEventTitle(text) || '提醒';
  if (cat === 'business_trip' && !title.includes('出差')) title = `出差·${title}`;
  if (cat === 'meeting' && !/(会议|会)/.test(title)) title = `会议·${title}`;
  if (cat === 'travel' && !/(出行|出发|赶)/.test(title)) title = `出行·${title}`;
  const r = createReminder(title, parsed.iso, {
    repeat: parsed.repeat, repeatWeekday: parsed.repeatWeekday, repeatMonthDay: parsed.repeatMonthDay,
    category: cat, method, source: 'chat'
  }, userId);
  db.forUser(userId).pushNotification({ text: `已添加提醒：${title}（${parsed.label}，${methodLabel}）`, level: 'success' });
  let reply = `已添加提醒：「${title}」${REMINDER_CATEGORIES[cat].icon} ⏰ ${parsed.label} · ${methodLabel}`;
  if (parsed.dateOnly) reply += '（时间默认 09:00，可在「我的」页修改）';
  reply += '，已加入「我的」页面。';
  const suggestions = cat === 'business_trip' ? ['顺便规划出差行程', '查看我的提醒']
    : cat === 'travel' ? ['顺便规划行程', '查看我的提醒']
    : ['查看我的提醒', '再添加一个'];
...
async function diaryFlow(text, userId) {
  ...
  const dp = cnParts(new Date(date));
  const title = topic ? `${topic}日记` : `${dp.m}月${dp.day}日日记`;
